...
object SVM {
  ...
  def sgd_subset(train_XY: (Int, (Map[Int, Float], Int)), W: mutable.Map[Int, Double], regParam: Double, D: Int) =  {
  ...
  def compute_gradient(xn: Map[Int, Float], yn: Double, wsub: mutable.Map[Int, Double], regParam: Double, D: Int) = {
  ...
  def is_support(yn: Double, xn: Map[Int, Float], w: mutable.Map[Int, Double]) = {
    /* Checks if a datapoint is support */

    yn * xn.map(x => w(x._1) * x._2).sum < 1
  }

  def compute_loss(train_XY: Vector[(Int, (Map[Int, Float], Int))], W: mutable.Map[Int, Double], regParam: Double): Double = {
  ...
  def hinge_loss(XY: Vector[(Int, (Map[Int, Float], Int))], w: mutable.Map[Int, Double]) = {
